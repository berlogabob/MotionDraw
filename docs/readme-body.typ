...
+ Optional tempo:
  ...
  - #strong[TEMPO : SEQ] --- detections are played back one note per
    tick. `SEQ : LOOP` appends them to a looping step sequence: empty
    steps fill first, then the oldest step is overwritten, so the phrase
    keeps following the scene; `LEN` sets the loop length. `SEQ : ONCE`
    queues them instead: each tick plays the oldest one and it is gone,
    so a burst becomes a run spread over time and the line goes silent
    when the queue empties. `CLEAR` empties either.
